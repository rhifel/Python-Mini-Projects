...
class WeatherApp(QWidget):
    ...
    def initUI(self):
        ...
        vbox = QVBoxLayout()
        ui_elements = [(self.city_label, "city_label"), 
                       (self.city_input, "city_input"),
                       (self.get_weather_button, "weather"),
                       (self.temperature_label, "temp"),
                       (self.emoji_label, "emoji"),
                       (self.description_label, "description"),] # store in a list 
        # changed it into a tuple
        
        for element, label in ui_elements: # use for loop for better structure
            vbox.addWidget(element) 
            element.setObjectName(label)
    ...
    def set_weather_icon(self, icon_path, size=150):
        ...
        if not pixmap.isNull():
            ...
            self.emoji_label.clear() # forgot to mention i used pixmap here idk it still looks low quality
